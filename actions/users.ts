...
export async function createUser(state: CreateUserState, formData: FormData){
    ...
    try {
        await sql`INSERT INTO USUARIO (USU_STR_NOME, USU_STR_EMAIL, USU_STR_SENHA, USU_STR_TEL, USU_STR_CPF, USU_STR_ROLE) VALUES (${name}, ${email}, ${hashedPassword}, ${telefone}, ${cpf}, ${role})`
        console.log(`'${name}', '${email}', ${hashedPassword}, ${telefone}, ${cpf}, ${role}`)
    } catch (error) {
